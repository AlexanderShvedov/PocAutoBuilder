...
public class Main {
    public static void main(String[] args) throws NamingException, IOException, SAXException {
        JMSServiceAdapter adapter = new JMSServiceAdapter();
        XMap src = new XMap(Files.newInputStream(Paths.get("Graniteds-adapter-PoC.xml")));
        XMap dest = new XMap(Files.newInputStream(Paths.get("Graniteds-dest-PoC.xml")));
        adapter.configure(src, dest);
    }
}